"""
ALPACA AUTOMATED TRADING BOT - WITH 3-DAY MAX HOLD
===================================================
...
✓ Closes positions after 3 days max (even if target/stop not hit)
✓ Sends email alerts
...
MAX_HOLD_DAYS = 3  # Close positions after 3 days
...
class AutoTrader:
    ...
    def manage_positions_3day_hold(self):
        """Close any positions older than 3 days"""
        print(f"\n[{self._time()}] Checking for positions past 3-day hold...")
        ...
        for position in positions:
            try:
                ...
                # Get entry time from position (Alpaca tracks this)
                # Note: Alpaca's position entry_date is a string, parse it
                entry_time_str = position.entered_at
                ...
                if entry_time_str:
                    ...
                    # If position is older than 3 days, close it
                    if days_held >= MAX_HOLD_DAYS:
                        print(f"  ⏱ {ticker}: Position held for {days_held} days (limit is {MAX_HOLD_DAYS})")
                        
                        # Cancel existing stop/target orders for this ticker
                        self.alpaca.cancel_all_orders(ticker)
                        print(f"    ├─ Cancelled stop/target orders")
                        
                        # Close position at market
                        closed = self.alpaca.close_position(ticker)
...
✓ Positions will AUTO-CLOSE after 3 days (if not closed by stop/target)
✓ No further action needed

MANAGEMENT:
- System will close at profit target OR stop loss (whichever hits first)
- If neither hits, positions will auto-close after 3 days
...
"""
    ...
    @staticmethod
    def send_position_closed_alert(closed_positions):
        """Send alert when positions are closed due to 3-day hold"""
        if not closed_positions:
            return
        
        print(f"\n[{Alerter._time()}] Sending 3-day hold alert...")
        
        subject = f"[AUTO CLOSE] {len(closed_positions)} Position(s) Closed - 3-Day Hold - {datetime.now().strftime('%Y-%m-%d')}"
        
        body = f"""
AUTOMATED TRADING BOT - 3-DAY HOLD CLOSURE
Generated: {datetime.now().strftime('%Y-%m-%d %H:%M UTC')}

{len(closed_positions)} position(s) automatically closed due to 3-day hold limit:

"""
        
        for i, pos in enumerate(closed_positions, 1):
            body += f"""
Position #{i}: {pos['ticker']}
├─ Shares: {pos['shares']}
├─ Days Held: {pos['days_held']} days
└─ Closed: {pos['closed_time']} UTC
...
✓ Stop loss and profit target orders were cancelled
✓ System will now scan for new signals
...
def main():
    ...
    print(f"Max Hold: {MAX_HOLD_DAYS} days per position")
    print()
    ...
    try:
        ...
        # === STEP 1: MANAGE EXISTING POSITIONS (Close if 3+ days old) ===
        print("[STEP 1] Checking for positions past 3-day hold...")
        closed_positions = trader.manage_positions_3day_hold()
        if closed_positions:
            Alerter.send_position_closed_alert(closed_positions)
        ...
        print(f"✓ Closed {len(closed_positions)} old position(s)")
        if executed:
            print(f"✓ {len(executed)} new position(s) entered")
        else:
            print("✗ No new signals executed")
